...
function PDFInvoice({
  ...
  return {
    ...
    genTableHeaders: function genTableHeaders() {
      ['Description', 'Qty', 'Um', 'Price', 'Discounts', 'VatCode', 'Total', 'Notes'].forEach(function (text, i) {
        doc.fontSize(TEXT_SIZE_ITEMS).text(translate[text], table.x + i * table.inc, table.y);
      });
    },
    genTableRow: function genTableRow() {
      items.map(function (item) {
        ...
        ['Description', 'Qty', 'Um', 'Price', 'Discounts', 'VatCode', 'Total', 'Notes'].forEach(function (field, i) {
          doc.fontSize(TEXT_SIZE_ITEMS).text(item[field], table.x + i * table.inc, table.y + TEXT_SIZE + 6 + itemIndex * 20);
        });
      });
    },
